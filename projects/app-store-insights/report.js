...
(function () {
  ...
  function renderSnapshot() {
    ...
    document.getElementById("snapshot").innerHTML =
      ...
      "</div>" +
      '<p class="snapshot-note">These two ratings come from different populations — a recent ' +
        DATA.reviewSnapshot.reviewsAnalyzed + '-review sample vs. Apple\'s much larger all-time count — so the gap between them is shown for context, not as a trend or decline.</p>';
  }
  ...
  function renderWant() {
    ...
    document.getElementById("want-section").innerHTML =
      ...
      '<p class="report-lede">What people asked for, and the need underneath the ask — the two aren\'t always the same thing.</p>' +
      '<div class="content-cards">' + cards + "</div>";
  }
  ...
  function renderMethodology() {
    ...
    document.getElementById("methodology").innerHTML =
      '<h2 class="report-section-title">How This Report Was Generated</h2>' +
      '<div class="method-grid">' +
        fieldBlock("Data source", m.dataSource) +
        fieldBlock("Sample scope", m.sampleScope) +
        '<div class="field-block"><span class="field-label">Limitations</span><ul class="method-list">' +
          m.limitations.map(function (l) { return "<li>" + esc(l) + "</li>"; }).join("") +
        "</ul></div>" +
        fieldBlock("Deterministic facts vs. AI judgment", m.deterministicVsAI) +
        fieldBlock("Prioritization framework", m.prioritizationFramework) +
      "</div>";
  }
  ...
  function toMarkdown() {
    var d = DATA, lines = [];
    lines.push("# " + d.app.name + " — Product Insights\n");
    ...
    lines.push(d.methodology.dataSource + "\n");
    lines.push(d.methodology.sampleScope + "\n");
    lines.push("**Limitations:**");
    d.methodology.limitations.forEach(function (l) { lines.push("- " + l); });
    lines.push("\n" + d.methodology.deterministicVsAI + "\n");
    lines.push(d.methodology.prioritizationFramework);
